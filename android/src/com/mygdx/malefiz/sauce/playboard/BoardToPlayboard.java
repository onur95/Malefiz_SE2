package com.mygdx.malefiz.sauce.playboard;
...
import com.mygdx.malefiz.sauce.UpdateHandler;
import com.mygdx.malefiz.sauce.coordinates.CoordinateCalculation;
...
import com.mygdx.malefiz.sauce.field.FieldPosition;
import com.mygdx.malefiz.sauce.field.FieldStates;
import com.mygdx.malefiz.sauce.screens.LosingScreen;
import com.mygdx.malefiz.sauce.screens.WinningScreen;
...
public class BoardToPlayboard {
    ...
    private void setHighlight(int column, int row){
        ...
        field.addAction(action);
        field.addListener(new com.mygdx.malefiz.sauce.playboard.clicklistener.HighlightClickListener(column, row, stage.getActors().size, board, this, handler));
        stage.addActor(field);
    }
    ...
    private  void setPlayerHighlight(int column, int row, Coordinates coordinates){
        if(board.isPlayer(column, row)) {
            ...
            if (board.getBoardArray()[column][row].getFieldState().ordinal() == player.getNumber()) {
                field.addListener(new com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener(column, row, stage.getActors().size, player, this));
    ...
    private  Image getFieldType(int column, int row){
        ...
        if(field != null && board.getBoardArray()[column][row].getFieldState().ordinal() == player.getNumber()){
            ...
            field.addListener(new com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener(column, row, stage.getActors().size+1, player, this)); //Weil es muss ja auf das Highlight referenziert werden, das genau 1 darüber liegt
    ...
    public void adjustPlayerClickListener(int column, int row, int index){
        for(EventListener event : stage.getActors().get(index).getListeners()){
            if(event.getClass() == com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener.class){
                adjustPlayers(((com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener)event).getColumn(), ((com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener)event).getRow(), column, row);
                ((com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener)event).setColumn(column);
                ((com.mygdx.malefiz.sauce.playboard.clicklistener.PlayerClickListener)event).setRow(row);
            }
        }
    }
